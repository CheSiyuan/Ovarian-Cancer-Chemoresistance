...
naivebayes_model <- naiveBayes(label ~ ., data = train_data)

# Predict on training and validation sets with probabilities
prob_train <- predict(naivebayes_model, newdata = train_data, type = "raw")
prob_val <- predict(naivebayes_model, newdata = val_data, type = "raw")
...
pdf("roc_curves_naivebayes.pdf")
plot(roc_train, col = "blue", main = "ROC Curves for Naive Bayes Model", lwd = 2)
...
write(results, file = "naivebayes_model_results.txt")

# Print results
cat(results, "\n")
cat("ROC curves saved as roc_curves_naivebayes.pdf\n")
cat("Model results saved as naivebayes_model_results.txt\n")